...
def test_run_pipeline_disease_table_channel(tmp_path):
    """Smoke test: a CSV disease-biomarker table (disease, biomarker NAME,
    direction, source) feeds the channel end-to-end -- names resolve to
    HMDB, diseases to kept pathways, the audit CSV is written, and a
    direction-respecting spike flags the sample."""
    import numpy as np
    import yaml
    from pathway_pipeline.main import run_pipeline
    ...
    disease_table = tmp_path / "disease_biomarkers.csv"
    disease_table.write_text(
        "Disease,Biomarker,Direction,Source\n"
        "Alanine Metabolism,Adenosine triphosphate,up,literature-ref\n",
        encoding="utf-8")
    ...
    spike = rng.normal(1.0, 0.01, size=n)
    spike[-1] = 8.0
    # A second 'other' sample DECREASES ATP hard: direction 'up' must not
    # flag it.
    drop = rng.normal(1.0, 0.01, size=n)
    drop[-2] = -6.0
    data = {
        "Sample": [f"s{i}" for i in range(n)],
        "Classification": [0] * n_normal + [1] * n_other,
    ...
    decisions = result["sample_decisions"]
    # The audit CSV exists for review.
    audit = pd.read_csv(tmp_path / "out" / "disease_table_audit.csv")
    assert (audit["biomarker_match_method"] == "name_exact").any()
    assert (audit["disease_match_method"] == "exact").any()
    # The spiked sample flags through the biomarker channel.
    spiked = decisions[decisions["sample_id"] == f"s{n - 1}"].iloc[0]
    assert bool(spiked["biomarker_flagged"])
    # Channel columns are present.
    assert "biomarker_depth_p" in decisions.columns
    bio_flags = pd.read_csv(tmp_path / "out" / "biomarker_flags.csv")
    assert (bio_flags["hmdb_id"] == "HMDB0000538").any()
    assert set(bio_flags["direction"]) == {"up"}
