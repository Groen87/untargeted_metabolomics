...
def _run_scorer_sweep(
    ...
    """
    Sweep a single scorer hyperparameter (e.g. `nu` for OCSVM) at a fixed
    `n_components`, to map how the scorer's own hyperparameter trades off
    detection rate against FPR.

    Runs the full outer-CV pipeline once per value in `scorer_sweep_values`,
    overriding only `scorer_kwargs[<scorer_sweep_param>]` per iteration (the
    PCA `n_components` and the scorer selection stay fixed from the YAML).
    Each value uses outer k-fold CV (`cv_outer_folds`, or 5 if 1) for an honest
    out-of-sample estimate that averages out split noise. Outputs per value
    under sweep_scorer_<value>/, plus scorer_sweep.csv and scorer_sweep.png.

    Useful for tuning OCSVM's `nu` (the training-contamination fraction): at
    a fixed n_components it shows which `nu` keeps FPR near the deployment
    contamination while maximizing detection, decoupled from the components
    bias-variance tradeoff that `_run_components_sweep` maps.
    """
    param_name = config.get('scorer_sweep_param', 'nu')
    sweep_values = config.get_list('scorer_sweep_values', [])
    if not sweep_values:
        sweep_values = [0.01, 0.02, 0.05, 0.1, 0.2, 0.3, 0.5]
    # Preserve the declared type so OCSVM's `nu` stays a float rather than
    # being stringified by the YAML loader.
    sweep_values = [_coerce_sweep_value(v, param_name) for v in sweep_values]
    ...
    # Snapshot config so we can restore it after the sweep.
    orig_cv_folds = config.get('cv_outer_folds', 1)
    orig_scorer_kwargs = config.get('scorer_kwargs', None)
    orig_eval_strategy = config.get('evaluation_strategy', 'realistic')
    # The sweep measures detection rate / FPR, which only the realistic
    # evaluation path produces. Force it for the sweep iterations.
    config.set('evaluation_strategy', 'realistic')
    config.set('cv_outer_folds', n_folds)
    ...
    logger.info(f"Fixed n_components = {config.get('n_components', 100)}, "
                f"scorer = {scorer_name}. Each value uses outer CV with "
                f"{n_folds} folds. Outputs per value under "
                f"sweep_scorer_<value>/.")

    sweep_rows = []
    for val in sweep_values:
        _log_section_header(f"SWEEP POINT: {scorer_name}.{param_name} = {val}")
        iter_kwargs = dict(base_kwargs)
        iter_kwargs[param_name] = val
        config.set('scorer_kwargs', iter_kwargs)

        # Stable directory name per value (avoids float-in-pathname quirks).
        val_tag = _sweep_value_tag(val)
        sweep_dir = output_dir / f"sweep_scorer_{val_tag}"
        sweep_dir.mkdir(parents=True, exist_ok=True)
        ...
        sweep_rows.append(row)
        logger.info(f"SWEEP POINT {param_name}={val}: "
                    f"detection={row['detection_rate']:.3f}, "
                    f"FPR={row['false_positive_rate']:.3f}, "
                    f"roc_auc={row['roc_auc']:.3f}")
    ...
    # Restore the original config values.
    config.set('cv_outer_folds', orig_cv_folds)
    config.set('scorer_kwargs', orig_scorer_kwargs)
    config.set('evaluation_strategy', orig_eval_strategy)

    sweep_df = pd.DataFrame(sweep_rows)
    sweep_df.to_csv(output_dir / "scorer_sweep.csv", index=False)
    logger.info(f"Scorer sweep table saved to {output_dir / 'scorer_sweep.csv'}")

    _log_section_header(
        f"SCORER SWEEP SUMMARY ({scorer_name}: detection vs FPR vs {param_name})"
    )
    logger.info(f"{param_name:>10} {'detect':>8} {'FPR':>8} {'roc_auc':>8} "
                f"{'prec':>8} {'f1':>8}")
    for _, r in sweep_df.iterrows():
        logger.info(f"{str(r[param_name]):>10} "
                    f"{r['detection_rate']:>8.3f} "
                    f"{r['false_positive_rate']:>8.3f} "
                    f"{r['roc_auc']:>8.3f} "
                    f"{r['precision']:>8.3f} "
    ...
    # Detection-vs-FPR tradeoff plot over the swept scorer param.
    try:
        import matplotlib
        matplotlib.use("Agg")
        import matplotlib.pyplot as plt
        fig, ax1 = plt.subplots(figsize=(10, 6))
        x = [str(r[param_name]) for r in sweep_rows]
        ax1.plot(x, sweep_df['detection_rate'].values, 'o-',
                 color='tab:red', label='Detection rate (recall)')
        ax1.plot(x, sweep_df['false_positive_rate'].values, 's--',
                 color='tab:blue', label='False positive rate')
        ax1.set_xlabel(f'{scorer_name}.{param_name}')
        ax1.set_ylabel('Rate')
        ax1.set_ylim(0, 1)
        ax1.axhline(config.get('realistic_test_contamination', 0.02),
                    color='tab:blue', linestyle=':', alpha=0.5,
                    label='Nominal contamination (2%)')
        ax1.set_title(f'{scorer_name} sweep: detection rate vs FPR ({param_name})')
        ax1.legend(loc='center right')
        ax1.grid(True, alpha=0.3)
        ...
        fig.tight_layout()
        fig.savefig(output_dir / "scorer_sweep.png", dpi=300,
                    bbox_inches='tight')
        plt.close(fig)
        logger.info(f"Scorer sweep plot saved to {output_dir / 'scorer_sweep.png'}")
    except Exception as e:
        logger.warning(f"Could not render scorer sweep plot: {e}")

    return {
        'evaluation_strategy': 'scorer_sweep',
...
def _coerce_sweep_value(val: Any, param_name: str) -> Any:
    """Coerce a YAML sweep value to the type expected for `param_name`.

    OCSVM's `nu` and `gamma` (when numeric) are floats; tree/distance params
    like `n_estimators`, `n_neighbors`, `max_samples`, `n_components` are
    ints. Anything else is left as-is. This guards against a YAML loader that
    may hand back a string (e.g. '0.05') for a value that should be numeric.
    """
    int_params = {'n_estimators', 'n_neighbors', 'n_components', 'max_samples'}
    if isinstance(val, str):
        ...
        try:
            ...
            return val
    if param_name in int_params and isinstance(val, float) and val.is_integer():
        return int(val)
    return val


def _sweep_value_tag(val: Any) -> str:
    """Filesystem-safe tag for a swept value (used in output dir names)."""
...
def run_pipeline(
    ...
    if evaluation_strategy == 'scorer_sweep':
        # Sweep a single scorer hyperparameter (e.g. OCSVM `nu`) at a fixed
        # n_components to map the detection-rate vs FPR tradeoff over the
        # scorer's own knob. Runs the full outer-CV pipeline once per value in
        # scorer_sweep_values and saves a detection-vs-FPR curve. Set
        # `evaluation_strategy: scorer_sweep` in the YAML to enable; the kwarg
        # to sweep is `scorer_sweep_param` (default `nu`).
        return _run_scorer_sweep(
            features=features,
            classification=classification,
            normal_class=normal_class,
            outlier_classes=outlier_classes,
