...
class SparsePCAWrapper:
    ...
    def _get_numpy_array(self, X: Union[pd.DataFrame, np.ndarray]) -> np.ndarray:
    ...
    def _fit_regular_pca(self, X: Union[pd.DataFrame, np.ndarray]) -> Tuple[np.ndarray, np.ndarray]:
        ...
        X_array = self._get_numpy_array(X)
    ...
    def _get_fit_method(self, method: str):
        """Get the appropriate fit method."""
        methods = {
            'regular': self._fit_regular_pca,
            'sparse': self._fit_sparse_pca,
            'mini_batch_sparse': self._fit_mini_batch_sparse_pca,
            'two_phase': self._fit_two_phase_pca,
        }
        if method not in methods:
            raise ValueError(
                f"Unknown PCA method: {method}. "
                f"Use 'regular', 'sparse', 'mini_batch_sparse', or 'two_phase'."
            )
    ...
    def fit_transform(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        fit_method = self._get_fit_method(self.method)
        X_transformed, explained_variance = fit_method(X)
    ...
    def fit(self, X: Union[pd.DataFrame, np.ndarray]) -> "SparsePCAWrapper":
        ...
        X_index = X.index if isinstance(X, pd.DataFrame) else None
        X_values = self._get_numpy_array(X)
        ...
        fit_method = self._get_fit_method(self.method)
        X_transformed, explained_variance = fit_method(X_values)
