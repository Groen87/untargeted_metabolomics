#!/usr/bin/env python3
"""Main script to run the metabolomics data processing pipeline for NEG and POS modes."""
...
def main():
    """Run the pipeline for NEG and POS ion modes, ending after ComBat."""
    try:
        # Load config
        config = Config()
        ...
        for mode in ["NEG", "POS"]:
            ...
            if reference_batch:
                ref_data_file = Path(f"data/{reference_batch}/output/{mode}/pqn_normalized_data.csv")
                ...
                if ref_data_file.exists() and ref_batch_file.exists():
                    ...
                    # Merge batches
                    merged_data, merged_batch = merge_batches_for_ralps(
                        drift_corrected_file_batch1=str(output_dir / "pqn_normalized_data.csv"),
                        ...
                        ralps_input_dir=combat_input_dir,
                        ralps_output_dir=combat_output_dir,
                    ...
                    merged_data = pd.read_csv(str(combat_input_dir / "merged_data_for_ralps.csv"), index_col=0)
                    merged_batch = pd.read_csv(str(combat_input_dir / "merged_batch_for_ralps.csv"))
                    print(f"Merged data shape: {merged_data.shape} (features x samples)")
                    print(f"Merged batch shape: {merged_batch.shape} (samples x metadata)")

                    # Step 5: Run ComBat
                    logger.info(f"[{mode}] Running ComBat batch correction...")
                    combat_corrected_df, combat_metrics = run_combat_and_visualize(
                        merged_data_path=str(combat_input_dir / "merged_data_for_ralps.csv"),
                        merged_batch_path=str(combat_input_dir / "merged_batch_for_ralps.csv"),
                    ...
                    logger.warning(f"[{mode}] Reference batch files not found. Skipping ComBat.")
            else:
