...
def main():
    ...
    try:
        ...
        for mode in ["NEG", "POS"]:
            ...
            # Step 3: Normalization (PQN or Median based on config)
            norm_method = config.get('normalization_method', 'pqn').lower()
            ...
            if reference_batch:
                norm_suffix = 'median_normalized' if norm_method == 'median' else 'pqn_normalized'
                ref_data_file = Path(f"data/{reference_batch}/output/{mode}/{norm_suffix}.csv")
                ...
                if ref_data_file.exists() and ref_batch_file.exists():
                    ...
                    merged_data, merged_batch = merge_batches_for_combat(
                        drift_corrected_file_batch1=str(output_dir / f"{norm_suffix}.csv"),
