#!/usr/bin/env python3
"""Clean, simple entry point for pathway analysis.

This script implements a COMPLETELY NEW, streamlined pathway analysis that:
...
This is a FRESH START - completely separate from the buggy enhanced pipeline.

Usage:
    python run_clean_analysis.py --input combined_batch_pipeline/dummy.csv --output outputs/clean_test
"""
...
from pathway_pipeline.pipeline.pathway_analysis_clean import run_clean_pathway_analysis
...
def _setup_logging(output_dir: Path) -> None:
    ...
    fh = logging.FileHandler(output_dir / "clean_analysis.log")
...
def create_simple_mapping(features: pd.DataFrame) -> pd.DataFrame:
    ...
    for feature in features.columns:
        ...
        elif 'GLYCINE' in feature_upper:
            pathway_name = "Glycine Metabolism"
        elif 'CHOLINE' in feature_upper:
            pathway_name = "Choline Metabolism"
        elif 'TAURINE' in feature_upper:
...
def compute_zscores(features: pd.DataFrame, metadata: pd.DataFrame) -> pd.DataFrame:
    """Compute robust z-scores for all features.
    
    Uses median and IQR computed on the normal reference set.
    """
    ...
    zscores = pd.DataFrame(index=features.index, columns=features.columns)
    for feature in features.columns:
        normal_values = features.loc[normal_mask, feature]
        sample_values = features[feature]
...
def main():
    parser = argparse.ArgumentParser(
        description="Clean pathway analysis: feature -> pathway -> Stouffer's Z -> flag"
    )
    parser.add_argument("--input", default="combined_batch_pipeline/dummy.csv",
                        help="Path to the feature matrix CSV.")
    parser.add_argument("--output", default="outputs/clean_analysis",
    ...
    zscores = compute_zscores(features_filtered, metadata)
    
    # Save z-scores
    zscores.to_csv(out / "metabolite_zscores.csv")
    
    # Step 5: Run clean pathway analysis
    logger.info("\n" + "="*70)
    logger.info("STEP 5: Run clean pathway analysis")
    logger.info("="*70)
    
    results = run_clean_pathway_analysis(
        zscores=zscores,
        feature_to_pathway=feature_to_pathway,
        metadata=metadata,
        output_dir=out,
        min_pathway_size=1,  # Lower since we have few features per pathway
